...
void e_init_motors(void)
{
	...
	/* set a higher priority on motors' interrupts, so that driving doesn't get disturbed by bluetooth communication */	
	...
	IPC5 = (IPC5 & 0xF00F) + 0x0660; //Setting interrupt priority of timers T4 and T5 to value 6. (0 = lowest priority, 7=highest priority, 4=default)
	//IPC5bits.T5IP=6; // Timer T5 priority = 6; Same as above, another notation
	//IPC5bits.T4IP=6; // Timer T4 priority = 6; Same as above, another notation
}
