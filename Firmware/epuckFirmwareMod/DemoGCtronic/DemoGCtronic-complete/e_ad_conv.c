...
void e_init_ad_scan(unsigned char only_micro)
{
	...
	/* set a higher priority on AD scan interrupts, so that the microphone sampling frequency stays the same, even when other processes are runnning */	
	// ATr: See "TABLE 5-2: INTERRUPT CONTROLLER REGISTER MAP" in "dsPIC30F6011A/6012A/6013A/6014A Manual" to make sense of the next line
	IPC2 = (IPC2 & 0x0FFF) + 0x6000; //Setting interrupt priority of ADC to value 6. (0 = lowest priority, 7=highest priority, 4=default)
	//IPC2bits.ADIP=6; // ADC priority = 6; Same as above, another notation
	//Note: Natural interrupt order of ADC is higher, than Timer4 and Timer5 (Motors). -> ADC has higher effectove priority than motors, although both have priority level 6 (see e_motors.c)
