...
void led_show_8bit(unsigned int number)
{
	e_set_led(0, (0x01 & number)!=0);
	e_set_led(1, (0x02 & number)!=0);
	e_set_led(2, (0x04 & number)!=0);
	e_set_led(3, (0x08 & number)!=0);
	e_set_led(4, (0x10 & number)!=0);
	e_set_led(5, (0x20 & number)!=0);
	e_set_led(6, (0x40 & number)!=0);
	e_set_led(7, (0x80 & number)!=0);
}

void led_show_angle(float alpha)
{
	if (alpha >= 0.0)
	{
		alpha = fmodf(alpha+3.1416, 6.2832)-3.1416; //Making sure the angle is inside the interval [-PI,PI]
	} else {
		alpha = fmodf(alpha-3.1416, 6.2832)+3.1416; //Making sure the angle is inside the interval [-PI,PI]
	}

	//e_led_clear(); cannot clear if also using led_show_angle_blink()

	if (alpha<-2.7489) //-(7/8)*PI
	{
		LED4 = 1;
	} else if (alpha<-1.9635) //-(5/8)*PI
	{
		LED3 = 1;
	} else if (alpha<-1.1781) //-(3/8)*PI
	{
		LED2 = 1;
	} else if (alpha<-0.3927) //-(1/8)*PI
	{
		LED1 = 1;
	} else if (alpha<0.3927) //+(1/8)*PI
	{
...
void led_show_angle_blink(float alpha)
{
	if (alpha >= 0.0)
	{
		alpha = fmodf(alpha+3.1416, 6.2832)-3.1416; //Making sure the angle is inside the interval [-PI,PI]
